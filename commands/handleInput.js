...
// FIX: Only deletes the EMOJI message after 1.5s.
// Never touches replyToMsgId (batter's number) — that stays permanently.
// Fire-and-forget — no await needed, emoji is purely cosmetic.
function sendDisappearingEmoji(groupId, replyToMsgId, emojiKey) {
  const emoji = RESULT_EMOJI[emojiKey];
  if (!emoji) return;
  bot.telegram.sendMessage(groupId, emoji, {
    reply_to_message_id:         replyToMsgId,
    allow_sending_without_reply: true,
  }).then(sent => {
    // Only delete the emoji itself — NOT replyToMsgId
    setTimeout(() => {
...
// scheduleCleanup REMOVED — it was deleting the batter's number message
...
function startTurnTimer(match, type) {
  match.warning30 = setTimeout(() => {
    if ((type === "bowl" && match.awaitingBowl) ||
      ...
      bot.telegram.sendMessage(match.groupId, `${ping} ⏳ 30s left`, { parse_mode: "HTML" }).catch(() => {});
    }
  }, 30000);
  ...
  match.warning10 = setTimeout(() => {
    if ((type === "bowl" && match.awaitingBowl) ||
      ...
      const playerName = getName(match, playerId);
      const ping       = playerId ? `<a href="tg://user?id=${playerId}">${playerName}</a>` : "";
      bot.telegram.sendMessage(match.groupId, `${ping} 🚨 10s left`, { parse_mode: "HTML" }).catch(() => {});
    }
  }, 50000);
...
async function announceBall(match) {
  ...
  // PERF: group message and bowler DM sent simultaneously
  const groupSend = bowlingGif
    ? bot.telegram.sendVideo(match.groupId, bowlingGif, {
        caption: bowlCaption, parse_mode: "HTML", supports_streaming: true, ...bowlingOpts
      }).catch(() =>
        bot.telegram.sendMessage(match.groupId, bowlCaption, { parse_mode: "HTML", ...bowlingOpts })
      )
    : bot.telegram.sendMessage(match.groupId, bowlCaption, { parse_mode: "HTML", ...bowlingOpts });
  ...
  const dmSend = bot.telegram.sendVideo(match.bowler, dmGif, {
    caption: dmCaption, supports_streaming: true
  }).catch(() =>
  ...
  await Promise.all([groupSend, dmSend]);
}
...
async function processBall(match) {
  ...
  try {
    ...
    if (bat === bowl) {
      ...
      // FIX: fire-and-forget, only emoji is deleted after 1.5s — batter msg untouched
      sendDisappearingEmoji(match.groupId, match.strikerMessageId, "skull");
      ...
      if (isDuck) {
        ...
        if (match.duckStreak >= 3) {
          bot.telegram.sendMessage(match.groupId, randomMilestoneLine('duckHattrick')).catch(() => {});
          match.duckStreak = 0;
        } else {
      ...
      if      (bowlerWkts === 3) bot.telegram.sendMessage(match.groupId, randomMilestoneLine('threeFer')).catch(() => {});
      else if (bowlerWkts === 4) bot.telegram.sendMessage(match.groupId, randomMilestoneLine('fourFer')).catch(() => {});
      else if (bowlerWkts === 5) bot.telegram.sendMessage(match.groupId, randomMilestoneLine('fiveFer')).catch(() => {});
      else if (bowlerWkts >= 6)  bot.telegram.sendMessage(match.groupId, randomMilestoneLine('sixFer')).catch(() => {});

      if (isHattrick) {
        const hattrickCall = getHattrickCall();
        if (hattrickCall.gif) {
          const method = hattrickCall.gif.startsWith("BAAC") ? "sendVideo" : "sendAnimation";
          bot.telegram[method](match.groupId, hattrickCall.gif, { caption: hattrickCall.text })
            .catch(() => bot.telegram.sendMessage(match.groupId, hattrickCall.text).catch(() => {}));
        } else {
          bot.telegram.sendMessage(match.groupId, hattrickCall.text).catch(() => {});
        }
    ...
    // FIX: fire-and-forget, only emoji deleted after 1.5s — batter msg untouched
    sendDisappearingEmoji(match.groupId, match.strikerMessageId, getResultEmoji(bat, false));

    // PERF: commentary + milestones in parallel
    const sends = [];

    if (match.currentPartnershipRuns === 50)
      sends.push(sendWithGif(match.groupId, 'partnership', randomMilestoneLine('partnership50')));
    else if (match.currentPartnershipRuns === 100)
      sends.push(sendWithGif(match.groupId, 'partnership', randomMilestoneLine('partnership100')));

    sends.push(sendWithGif(match.groupId, bat, randomLine(bat)));

    const bRuns       = match.batterStats[match.striker]?.runs ?? 0;
    const bRunsBefore = bRuns - bat;
    if (bRunsBefore < 50 && bRuns >= 50 && bRuns < 100)
      sends.push(sendWithGif(match.groupId, 'fifty', randomMilestoneLine('fifty')));
    else if (bRunsBefore < 100 && bRuns >= 100)
      sends.push(sendWithGif(match.groupId, 'hundred', randomMilestoneLine('hundred')));
