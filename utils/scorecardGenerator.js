...
function generateScorecard(match) {
  ...
  if (match.innings === 2 && match.target) {
    ...
    if (ballsLeft > 0) {
      ...
      rrrLine = `RRR: ${rrr} | Need ${runsNeeded} from ${ballsLeft} balls`;
    }
  }
  ...
  const strikerName =
    getName(match, match.striker).padEnd(14," ");

  const nonStrikerName =
    getName(match, match.nonStriker).padEnd(14," ");

  batters += `${strikerName} ⭐ ${strikerStats.runs}(${strikerStats.balls})\n`;
  batters += `${nonStrikerName}   ${nonStrikerStats.runs}(${nonStrikerStats.balls})\n`;
  ...
  for (const id in match.bowlerStats) {
    ...
    const oversHistory = match.overHistory
      .filter(o => o.bowler == id);
